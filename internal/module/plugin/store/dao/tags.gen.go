...
func newTag(db *gorm.DB, opts ...gen.DOOption) tag {
	...
	_tag.Plugins = tagManyToManyPlugins{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Plugins", "model.Plugin"),
		Versions: struct {
			field.RelationField
		}{
			RelationField: field.NewRelation("Plugins.Versions", "model.Version"),
		},
		Tags: struct {
			field.RelationField
			Plugins struct {
			...
			RelationField: field.NewRelation("Plugins.Tags", "model.Tag"),
			Plugins: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Plugins.Tags.Plugins", "model.Plugin"),
			},
		},
	}
...
type tagManyToManyPlugins struct {
	db *gorm.DB
...
func (a tagManyToManyPlugins) Where(conds ...field.Expr) *tagManyToManyPlugins {
	if len(conds) == 0 {
		return &a
	}
...
func (a tagManyToManyPlugins) WithContext(ctx context.Context) *tagManyToManyPlugins {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a tagManyToManyPlugins) Session(session *gorm.Session) *tagManyToManyPlugins {
	a.db = a.db.Session(session)
	return &a
}

func (a tagManyToManyPlugins) Model(m *model.Tag) *tagManyToManyPluginsTx {
	return &tagManyToManyPluginsTx{a.db.Model(m).Association(a.Name())}
}

type tagManyToManyPluginsTx struct{ tx *gorm.Association }

func (a tagManyToManyPluginsTx) Find() (result []*model.Plugin, err error) {
	return result, a.tx.Find(&result)
}

func (a tagManyToManyPluginsTx) Append(values ...*model.Plugin) (err error) {
...
func (a tagManyToManyPluginsTx) Replace(values ...*model.Plugin) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a tagManyToManyPluginsTx) Delete(values ...*model.Plugin) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a tagManyToManyPluginsTx) Clear() error {
	return a.tx.Clear()
}

func (a tagManyToManyPluginsTx) Count() int64 {
	return a.tx.Count()
}
